...
#define pin_433_marche 32 // Marche Direction télécommande 433mhz
#define pin_433_MD 34 // Arret Vitesse télécommande 433mhz
#define pin_433_reset 35 // Marche Vitesse télécommande 433mhz
#define pin_433_arret 33 // Arret Direction télécommande 433mhz
...
int d_bord = 200; // distance du bord droit
...
void setup() {
  ...
  d_bord = ee_read(24); // distance du bord droit
  ...
  // télécommande 
  pinMode(pin_433_marche, INPUT); // A marche direction
  pinMode(pin_433_reset, INPUT); // B marche vitesse
  pinMode(pin_433_arret, INPUT); // C arret direction  
  pinMode(pin_433_MD, INPUT); // D arret vitesse 
...
void loop() {
 ...
 //////////////////////////////// direction  ////////////////////////////////////
  ...
  direction = ( distance_droit - d_bord )/coef_direction;
 ...
 // Serial.println(direction);
  ...
  if      (digitalRead(pin_433_marche)) {// A
            marche_dir = true; 
            marche_vit = true;
          }
  else if (digitalRead(pin_433_MD)) {// B
            marche_dir = true;           
          }           
  else if (digitalRead(pin_433_reset)) {// D 
            nb_puls = 0;  // reset l'arret de la voiture
          } 
  else if (digitalRead(pin_433_arret)){ // c 
...
void init_lidar(){
    ...
    delay(100);
      ...
      lidar_centre.setMeasurementTimingBudget(50000);  // 13000
      ...
      lidar_centre.startContinuous(50);   // 13 

      delay(100);
      digitalWrite(18,1);
      delay(100);
      
      lidar_droit.setMeasurementTimingBudget(50000);   // 13000
      ...
      lidar_droit.startContinuous(50);   /// 13
...
void wifi(){
    ...
    else if (request.indexOf("?d_bord") != -1 ) { // si la requette est /led
      d_bord = val(request);
      ee_write(d_bord , 24); // enregistre d_gauche dans la EEprom à l'adresse 8
...
void p_2(){ // parametre voiture
  ...
  input("Distance du bord droit\n" , d_bord, "d_bord"); // input(texte, valeur à modifier, nom de la requette)
...
void entete(String texte, int temps){// header texte=titre de la fenetre, r = au temps en seconde avant de rafraichir la fenetre
  ...
  p += "</head>\n";
  p += "<body><center>";
  p += "<h1><p style=\"background-color: #000000; color: white;\">" + texte + "<br></p>";
  p += "</h1>";
}
...
void titre2(String texte, String cf, String ct){ // h=taille du texte, cf=couleur du fond, ct=couleur du texte
  p += "<p style=\"white-space:pre;background-color:"+ cf +"; color: "+ ct +";\">"+texte+"</p>";
}
