...
void setup() {
  ...
  d_mini_dir = ee_read(14); // coef direction (5) eeprom adresse 10 
  ...
  d_maxi_dir = ee_read(24); // volume du son à jouer avec le bouton
...
*/
 ...
 //////////////////////////////// direction  ////////////////////////////////////
  direction = map(distance_droit, d_mini_dir, d_maxi_dir, d_gauche, d_droite); 
...
void wifi(){
    ...
    else if (request.indexOf("?d_mini_dir") != -1 ) { // si la requette est c_dir (coef_direction)
      d_mini_dir = val(request);
      ee_write(d_mini_dir , 14); // enregistre v_mini dans la EEprom à l'adresse 0
      p_2(); // affiche la page web 0
    }
    else if (request.indexOf("?d_maxi_dir") != -1 ) { // si la requette est c_dir (coef_direction)
      d_maxi_dir = val(request);
      ee_write(d_maxi_dir , 24); // enregistre v_mini dans la EEprom à l'adresse 0
      p_2(); // affiche la page web 0
    }    
...
void p_2(){ // parametre voiture
  ...
  input("Distance mini droit\n" ,d_mini_dir , "d_mini_dir"); // input(texte, valeur à modifier, nom de la requette)

  br(); br(); 

  input("Distance maxi droit\n" ,d_maxi_dir , "d_maxi_dir"); // input(texte, valeur à modifier, nom de la requette)
