...
void setup() {
  ...
  d_mini = ee_read(14); // coef direction (5) eeprom adresse 10 
  ...
  d_maxi = ee_read(24); // volume du son à jouer avec le bouton
...
*/
 ...
 //////////////////////////////// direction  ////////////////////////////////////
  direction = map(distance_droit, d_mini, d_maxi, d_gauche, d_droite); 
...
void wifi(){
    ...
    else if (request.indexOf("?d_mini") != -1 ) { // si la requette est c_dir (coef_direction)
      d_mini = val(request);
      ee_write(d_mini , 14); // enregistre v_mini dans la EEprom à l'adresse 0
      p_2(); // affiche la page web 0
    }
    else if (request.indexOf("?d_maxi") != -1 ) { // si la requette est c_dir (coef_direction)
      d_maxi = val(request);
      ee_write(d_maxi , 24); // enregistre v_mini dans la EEprom à l'adresse 0
      p_2(); // affiche la page web 0
    }    
...
void p_2(){ // parametre voiture
  ...
  input("Distance mini droit\n" ,d_mini , "d_mini"); // input(texte, valeur à modifier, nom de la requette)

  br(); br(); 

  input("Distance maxi droit\n" ,d_maxi , "d_maxi"); // input(texte, valeur à modifier, nom de la requette)
