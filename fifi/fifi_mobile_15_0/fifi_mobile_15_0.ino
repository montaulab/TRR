...
void setup() {
  ...
  v_acc = ee_read(40); // temps accélération
...
void loop() {
  ...
  if (distance_centre < d_mini_vit) {
  if (distance_droit > distance_dir - 50 and distance_droit < distance_dir + 50) vitesse = v_mini_centre;   
  else vitesse = v_mini;
  }
...
//  Serial.println(vitesse);
 ...
 //////////////////////////////// direction  ////////////////////////////////////
  ...
  if ( distance_centre < 100 and t0>trecul and marche_vit ) {
    recul = true;
    ...
    if (angle_recul>0) direction = angle_recul; 
  }
...
void wifi(){
    ...
    */
    ...
    else if (request.indexOf("?v_acc") != -1 ) { //  temps de recul
      v_acc = val(request);
      ee_write(v_acc , 40); //
      p_2(); // affiche la page web 3
    }  
...
void p_2(){ // parametre voiture
  ...
  input("vitesse accélération<br>" , v_acc, "v_acc"); // input(texte, valeur à modifier, nom de la requette)
